...
def compile_project(args):
    ...
    fondum_requirements_file = "{}/fondum_requirements.txt".format(args.library_path)
    orig_path = os.getcwd()
    ...
    if "general" in stg:
        with open("{}/jcfs_settings.py".format(destination_dir), "w") as pfile:
            ...
            pfile.write("# DO NOT EDIT. This file was autogenerated by fondum compile function.\n")
    ...
    #
    if VERBOSE:
        print("c09 compiling requirements.txt..")

    requirements = OrderedDict()
    with open(fondum_requirements_file) as f:
        for line in f:
            if "==" in line:
                lib, ver = line.split("==")
                requirements[lib] = ver
    if "custom_requirements.txt" in settings_files:
        with open("{}/{}".format(settings_dir, "custom_requirements.txt")) as f:
            for line in f:
                if "==" in line:
                    lib, ver = line.split("==")
                    requirements[lib] = ver
    else:
        ImportError("custom_requirements.txt file missing.")
    with open("{}/requirements.txt".format(destination_dir), "w") as f:
        for lib in requirements:
            f.write("{}=={}\n".format(lib, requirements[lib].strip()))


    if VERBOSE:
        print("c10 done with compile.")
